  ...
  <div class="admin-content">
    <div class="admin-content-body">
      <div class="am-cf am-padding am-padding-bottom-0">
        <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">入库历史</strong> / <small>InBound History</small></div>
      </div>
          ...
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <tr>
                ...
                <th>品类</th>
                <th>产地</th>
                <th style="text-align:center;">运煤车号</th>
                <th>数量/t</th>
                <th>单价/￥</th>
                <th>总价/￥</th>
                <th>确认人</th>
              ...
              <tbody>
                ...
                <c:if test="${status.index<fn:length(inBound_list)}">
				  <c:set var="inBound" value="${inBound_list[i]}" />
	              <tr>
	                <td align="center">${status.index + 1}</td>
	                <td align="center">${inBound.depotId }</td>
	                <td>${inBound.name }</td>
	                <td>${inBound.place }</td>
	                <td align="center">${inBound.truckId }</td>
	                <td>${inBound.amount }</td>
	                <td>${inBound.unitPrice }</td>
	                <td>${inBound.totalPrice }</td>
	                <td>${inBound.comfirmPerson }</td>
	                <td><fmt:formatDate value="${inBound.inBoundDate}" pattern="EEEE"/></td>
	                <td><fmt:formatDate value="${inBound.inBoundDate}" pattern="yyyy-MM-dd aa hh:mm"/></td>
	                <td>
	                	<c:if test="${'' != inBound.remark && null != inBound.remark }"><a style="cursor: crosshair; color: red;" onmouseover="showRemark(true, this);" onmouseout="showRemark(false, this);">异常</a></c:if>
	                	<c:if test="${'' == inBound.remark || null == inBound.remark }">正常</c:if>
	                  ...
	                  <div class="am-btn-toolbar">
	                    <div class="am-btn-group am-btn-group-xs">
	                      <button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" 
	                      		onclick="location.href='/coal-gas-system/inbound/detail?id=${inBound.id }'">
	                      ...
	                      <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" 
	                      		onclick="deleteInbound(${inBound.id })"><span class="am-icon-trash-o"></span> 删除</button>
	                    </div>
	                  </div>
	                </td>
	              </tr>
            ...
            <div class="am-cf">
              &nbsp;共 <fmt:formatNumber type="number" value="${fn:length(inBound_list) / 10 + 0.49 }" maxFractionDigits="0"/> 页 ${fn:length(inBound_list) } 条记录 -- 第 ${param.page } 页
              <div class="am-fr">
                <ul class="am-pagination">
                  ...
                  <c:if test="${param.page == 4 && (fn:length(inBound_list) / 10 - param.page) <= 1 }">
	                <c:forEach begin="0" end="4" var="i" varStatus="status">
                  ...
                  <c:if test="${param.page == 5 && (fn:length(inBound_list) / 10 - param.page) <= 0 }">
	                <c:forEach begin="0" end="4" var="i" varStatus="status">
				  ...
				  <c:if test="${fn:length(inBound_list) / 10 > 5 && (fn:length(inBound_list) / 10 - param.page) > 0 && (fn:length(inBound_list) / 10 - param.page) <= 1 }">
	                <c:forEach begin="0" end="4" var="i" varStatus="status">
					    <li <c:if test="${param.page == (param.page + status.index - 3) }">class="am-active"</c:if> ><a href="javascript:changePage(${param.page + status.index - 3 })">${param.page + status.index - 3 }</a></li>
					</c:forEach>
                  </c:if>
                  
				  <c:if test="${fn:length(inBound_list) / 10 > 5 && (fn:length(inBound_list) / 10 - param.page) > -1 && (fn:length(inBound_list) / 10 - param.page) <= 0 }">
	                <c:forEach begin="0" end="4" var="i" varStatus="status">
					    <li <c:if test="${param.page == (param.page + status.index - 4) }">class="am-active"</c:if> ><a href="javascript:changePage(${param.page + status.index - 4 })">${param.page + status.index - 4 }</a></li>
					</c:forEach>
                  </c:if>
                  
                  <li><a href="javascript:changePage(${fn:substringBefore(fn:length(inBound_list) / 10 + 0.99, '.') })">»</a></li>
                </ul>
              </div>
            </div>
...
<script type="text/javascript">
	...
	function changePage(page) {
		...
		if ((page - 1) * 10 < "${fn:length(inBound_list) }") {
	...
	function deleteInbound(id) {
		if ('A' == "${user.role}" || 'B' == "${user.role}") {
			if (window.confirm("确定删除此条记录？")) {
				var xhr = getXhr();
				xhr.open("get", "/coal-gas-system/inbound/delete?id=" + id, true);
